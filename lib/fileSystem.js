...
const FileSystem = class {
  ...
  _getSourceUrlByIdAndExtname(_id, extname) {
    const sourceUrl = path.join(this.prefixUrl, `${_id.toString()}${extname}`);
    return sourceUrl;
  }
  ...
  async createFromStream({ stream, _id, extname }) {
    // 组装参数 stream
    const target = path.join(this.directory, `${_id.toString()}${extname}`);
  ...
  async destroy({ _id, extname }) {
    const target = path.join(this.directory, `${_id}${extname}`);
